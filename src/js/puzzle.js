...
var Puzzle = function(game, pic, square) {
  ...
  this.won = false;
    ...
    this.src_image = this.game.add.image(Game.w, Game.h/2-100, pic);
    this.src_image.anchor.setTo(0.5);
    this.src_image.scale.setTo(0.5, 0.5);
    this.src_image.visible = false;

    // image preview
    this.img_preview = this.game.add.image(Game.w*1.05, Game.h*0.25, pic);
    this.img_preview.anchor.setTo(0.5);
    this.img_preview.scale.setTo(0.25, 0.25);
...
Puzzle.prototype = {
	...
	scatter: function() {
		for (var s=0; s < this.pieces.length;s++) {
			...
			piece.x = rand(Game.w/2, Game.w);
			piece.y = rand(Game.h/2, Game.h);
		}
	},
	destroy: function() {
		this.slots.forEach(function(slot) {
	...
	/*preview_toggle: function() {
		if (this.img_preview.visible === false) {
			this.img_preview.visible = true;
			this.game.world.bringToTop(this.img_preview);
			this.pieces.forEach(function(piece) {
				piece.visible = false;
			},this);
		}else {
			this.img_preview.visible = false;
			this.pieces.forEach(function(piece) {
				piece.visible = true;
			},this);
		}
